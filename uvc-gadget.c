...
v4l2_reqbufs_mmap(struct v4l2_device *dev, int nbufs)
{
	...
	printf("dev->v4l2_fd: 0x%08x req.count: %d", dev->v4l2_fd, req.count);
...
v4l2_process_data(struct v4l2_device *dev)
{
	...
	printf("dev->udev->uvc_fd: 0x%08x ubuf.length: %d ubuf.bytesused: %d ubuf.m.userptr: 0x%08x \n",
		dev->udev->uvc_fd,
		ubuf.length,
		ubuf.bytesused,
		ubuf.m.userptr);
...
main(int argc, char *argv[])
{
	...
	printf("if (!dummy_data_gen_mode && !mjpeg_image)\n");
	if (!dummy_data_gen_mode && !mjpeg_image) {
		/* UVC - V4L2 integrated path */
		vdev->nbufs = nbufs;
		...
		case IO_METHOD_MMAP:
			printf("Setting vdev io method to USERPTR\n");
			vdev->io = IO_METHOD_USERPTR;
			break;

		case IO_METHOD_USERPTR:
		default:
			printf("Setting vdev io method to MMAP\n");
